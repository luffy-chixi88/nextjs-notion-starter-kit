    ...
    return (
        <div className={cs('flex pt-20', className)}>
            ...
            <div className="ml-32">
                {list.map((item, i) => {
                    return (
                      <div key={i} className={cs('flex mb-10 cursor-pointer', { 'opacity-40': index !== i })} onClick={() => handleSlideTo(i)}>
                        <div className="mr-2">
                            <div className="bg-[var(--primary)] text-white rounded-full w-11 h-11 leading-[44px] text-center">{i+1}</div>
                        </div>
                        <div>
                            <h5 className="notion-h notion-h3 m-0" dangerouslySetInnerHTML={{__html: item.Name }}></h5>
